...
public class CoNLLInteractiveAnalysis implements CorefDocumentProcessor{
	...
	public CoNLLInteractiveAnalysis() {
	...
	public void process(int id, Document document) {
		...
		if (usePairs){
			...
			for (int i = 0; i < mentionList.size(); i++){
				...
				if (isCoreferent && m1.mentionType != MentionType.PRONOMINAL && !beSkipped.contains(m1.headString.toLowerCase())){
	...
	public static void main(String[] args){
		...
		try {
			...
			CoNLLInteractiveAnalysis analizer = new CoNLLInteractiveAnalysis();
			...
			Set<String> toBeRemovedFrequnet = new HashSet<String>();

			double freqThreshold = 5;//0.5 * analizer.allDoc; 
			for (String s : analizer.mentionDocCount.keySet()){
				if (analizer.mentionDocCount.get(s) > freqThreshold)
					veryFrequent.add(s);
			}
			String answer = "";
			System.out.println(veryFrequent.size());
			for (String s : veryFrequent){
				System.out.println(s);
				answer = userReader.nextLine();
				if (!answer.toLowerCase().startsWith("y")){
					toBeRemovedFrequnet.add(s);
					System.out.println("will be removed then");
				}

			}
			
			PrintWriter pw1 = new PrintWriter("SelectedFrequentWords");
			pw1.println(Arrays.toString(toBeRemovedFrequnet.toArray()));
			pw1.close();
...
//						merge = userReader.nextLine();
//
//					}
//					if (ratio >= 0.5 || merge.toLowerCase().startsWith("y")){
//						System.out.println("Merging then");
...
//			}
//
//			for (String s : similarDocs.keySet()){
//				pw.println(s + " " + Arrays.toString(similarDocs.get(s).toArray()));
//			}
//
//			pw.close();
		} catch (Exception e) {
