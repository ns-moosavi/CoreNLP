...
public class CorefUtils {
  ...
  public static List<Mention> getSortedGoldMentions(Document document) {
	    List<Mention> mentions = new ArrayList<>(document.predictedMentionsByID.values());
	    List<Mention> nonGold = new ArrayList<Mention>();
	    for (Mention m : mentions)
	    	if (!document.goldMentionsByID.containsKey(m.mentionID))
	    		nonGold.add(m);
	    mentions.removeAll(nonGold);
	    Collections.sort(mentions, (m1, m2) -> m1.appearEarlierThan(m2) ? -1 : 1);
	    return mentions;
	  }
  
  public static List<Pair<Integer, Integer>> getGoldMentionPairs(Document document) {
	     List<Pair<Integer, Integer>> pairs = new ArrayList<>();
	     List<Mention> mentions = getSortedGoldMentions(document);
  ...
  public static Map<Pair<Integer, Integer>, Boolean> getUnlabeledGoldMentionPairs(Document document) {
	     return CorefUtils.getGoldMentionPairs(document).stream()
	         .collect(Collectors.toMap(p -> p, p -> false));
	   }

	   public static Map<Pair<Integer, Integer>, Boolean> getLabeledGoldMentionPairs(Document document) {
	     Map<Pair<Integer, Integer>, Boolean> mentionPairs = getUnlabeledGoldMentionPairs(document);
